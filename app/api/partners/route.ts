...
export async function GET(request: Request) {
  try {
    ...
    const partners = await db.collection("partners").find(filter).sort({ name: 1 }).toArray()
...
export async function POST(request: Request) {
  try {
    ...
    const {
      name, type, status, city, wilayaId, email, phone, website, address, description, image, services, priceRange, rating, reviews
    } = body
